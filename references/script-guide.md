...
**visual anchor note → narration cue marker → annotated slide PNG (laser dot or red rectangle)**
...
- a target shape, exactly one of:
  - a point → `target: [x, y]` (drawn as a red laser dot); or
  - an area → `shape: rectangle` + `box: [x, y, w, h]` (drawn as a red
    rectangle, 2–3 px stroke). `x, y` is the top-left corner and `w, h` are
    normalized width/height, all between 0 and 1.

### Dot vs rectangle

Use `shape: rectangle` when you are highlighting an *area*: a table column or a
few rows, a whole chart/table, a key-value card, or any region that should be
spotlighted as one block.

Use the default laser dot when you are pointing at a *point*: a single button, a
single data point, a single label, a node in a diagram, or a small icon.
...
Before writing per-slide text, define the deck's story spine so each slide has a
job in a larger chain, and tag every slide with a `**Role:**`.

The story spine is the deck-level logic chain:

```text
Hook → Context → Method → Proof → Insight → Action
```

- `Hook` — why the audience should care;
- `Context` — the problem or current state;
- `Method` — how the deck addresses it;
- `Proof` — the key evidence (charts, tables, metrics);
- `Insight` — the main takeaway;
- `Action` — what the audience should do next.

Assign one role per slide from: `hook`, `context`, `method`, `proof`, `insight`,
`action`, `transition`, `reference`, `divider`. A slide's narration must advance
its role — a `proof` slide interprets evidence, a `transition` slide connects
ideas — rather than merely describing the slide.
...
- `**Role:**` states the slide's job in the story spine (one of the vocabulary).
...
- `**Visuals:**`, `**Pronounce:**`, and the story spine are reviewer metadata,
  not TTS text.
...
- Are any dots or rectangles likely to cover important content?
