...
# Stage 4.5 — Optional automated cue check (box → dot fallback)

```bash
VERIFY_BASE_URL=https://... VERIFY_API_KEY=... VERIFY_MODEL=<vision-model> \
python3 scripts/verify_cues.py $BUILD/cues $BUILD/visual_notes.yaml \
  --report $BUILD/verify_report.md
```

One vision call per cue image judges the placement against the anchor's
`element` text: `good`, `slightly_off` (accepted, nothing changes), or `bad`
(mostly off the element, covering a different block, or slicing through its
content). A `bad` **rectangle** anchor is downgraded to a laser dot at the
point the model reports inside the element; the result is written to
`visual_notes.verified.yaml` — re-run Stage 4 with that file. A `bad` dot
anchor is only reported for manual review.

The model may only *flag* cues, never move or reshape them. Without an endpoint
configured the script skips and tells you to check the cue images yourself. The
Stage 5 review gate still applies.
...
| cue rectangle clearly misplaced | run `verify_cues.py`: bad boxes are downgraded to dots automatically |
...
│   ├── verify_cues.py        cue-image QA; bad boxes → dots (optional check)
...
- `verify_cues.py` — one vision call per cue image; downgrades badly placed
  rectangles to dots and writes a report (optional; needs a vision endpoint);
