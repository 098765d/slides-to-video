...
def load_manifest(audio_dir: str, manifest_path: Optional[str]) -> List[Dict[str, object]]:
    p = manifest_path or os.path.join(audio_dir, "manifest.json")
    if not os.path.exists(p):
        sys.exit(f"no manifest at {p}; run tts_narration.py first")
...
def main() -> None:
    ...
    print(f"MODE=cue-aware SEGMENTS={len(segments)}")
